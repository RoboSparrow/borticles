...
static Rectangle _grid(unsigned int col, unsigned int row, unsigned int w, unsigned int h) {
    unsigned int col_width = (m_window.rect.width/2) - (3 * m_window.padding); // l + m + r

    unsigned int off_x = m_window.rect.x + m_window.padding;
    unsigned int off_y = m_window.rect.y + 30 + m_window.padding;

    return (Rectangle) {
        off_x + (col * col_width),
        off_y + (row * (m_window.line_height + m_window.line_spacing)),
        (w) ? w : col_width,
...
void ui_draw(State *state) {
    ...
    if (state->ui_minimized) {
        ...
        // paused
        GuiCheckBox(_grid(0, 0, m_window.line_height, 0), "Paused", &state->paused);

        // ui_debug
        GuiCheckBox(_grid(0, 1, m_window.line_height, 0), "Show debug", &state->ui_debug);

        // ui_borticles
        GuiCheckBox(_grid(0, 2, m_window.line_height, 0), "Show borticles", &state->ui_borticles);

        // ui_qtree
        GuiCheckBox(_grid(0, 3, m_window.line_height, 0), "Show quadtree", &state->ui_qtree);

        // pop_len
        float pop_len = (float) state->pop_len;
        GuiLabel(_grid(0, 4, 0, 0), "Population");
        GuiSliderBar(_grid(0, 5, 0, 0), NULL, TextFormat("%d (max:%d)", state->pop_len, state->pop_max), &pop_len, 0, state->pop_max);
        if (pop_len > 0.f && pop_len != state->pop_len) {
            state_set_pop_len(state, (int) round(pop_len));
        }

        // grav_g
        GuiLabel(_grid(0, 6, 0, 0), "Gravitational constant");
        GuiSlider(_grid(0, 7, 0, 0), NULL, TextFormat("%2.f (max:%2.f)", state->grav_g, 20.f), &grav_g, 0.f, 20.f);
